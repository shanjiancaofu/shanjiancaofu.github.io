---
title: RT-Thread内核----时钟管理
date: 2024-11-21 20:20:00 +0800
categories: [RT-Thread]
tags: [RT-Thread, 内核, 时钟管理, 定时器]
---
...
![定时器上下文环境](/assets/img/posts/rt-thread-clock/05timer_env.png)
...
![定时器链表示意图](/assets/img/posts/rt-thread-clock/05timer_linked_list.png)
...
![定时器链表插入示意图](/assets/img/posts/rt-thread-clock/05timer_linked_list2.png)
...
![有序链表示意图](/assets/img/posts/rt-thread-clock/05timer_skip_list.png)

使用跳表算法后可以采用类似二叉搜索树的方法，把一些节点提取出来作为索引，得到如下图所示的结构：

![有序链表索引示意图](/assets/img/posts/rt-thread-clock/05timer_skip_list2.png)

在这个结构里把 {3, 18,77} 提取出来作为一级索引，这样搜索的时候就可以减少比较次数了, 例如在搜索 39 时仅比较了 3 次（通过比较 3，18，39)。当然我们还可以再从一级索引提取一些元素出来，作为二级索引，这样更能加快元素搜索。

![三层跳表示意图](/assets/img/posts/rt-thread-clock/05timer_skip_list3.png)
...
![定时器相关操作](/assets/img/posts/rt-thread-clock/05timer_ops.png)
